...
3. possible to come up with a **state transtion equation**
...
1. **Visualize** - represent problem in terms of index (_DAG_)
2. **Identify Subproblem** (_a state_)
3. **Find relationship** among subproblems (_do all possible stuff on an index, state transition - count/min/max_)
4. **Generalize** the relationship (_recurrence relation_)
5. **Implement** - choose right order to solve subproblems (_memoization/tabulation_)


_Ref_:
- https://youtu.be/aPQY__2H3tE
- https://youtu.be/duSmc87gaUo
...
**Min cost climbing stairs** problem threw me off since we can start at either step `0` or `1`, and final step `n` isn't given, only step costs are given. Some observations:
- `cost[i]` is the cost of going further if we're at the `i`th step, destination is `n`th step. 
- If we assume the state as including cost of current step, then we directly jump from `n-1`th or `n-2`th step to `n`th step without any effective cost (as its already included in prev states). So for this problem `min(solve(n-1), solve(n-2))` can be one wey to solve instead of the usual `solve(n)`
- base cases become `n = 0, cost[0]`, and `n = 1, cost[1]` too since we just need `cost[1]` to go further from `1` (as we can start from `1` and go further at cost `cost[1]` without any prior steps)
...
**Identification of state** in min cost climbing stair problem: state here is modeled as cost of going further from state `i` (and it includes the cost of state `i`), instead of cost of arriving at state `i` (which can also be modeled)
...
4. [LC - House Robber](https://leetcode.com/problems/house-robber/) aka _Max Sum of Non-Adjacent Elements_ - base case `dp[0] = arr[0]` (since if we've reached  `0` index then that means we're coming from either `2` after picking it or `1` after not picking it so we always pick `0`; alternatively we can also understand this as - the robber robs the only house if there is a single house with loot amount as `dp[0]`), state to be pick (`p = arr[i] + dp[i - 2]`) and not pick (`np = 0 + dp[i - 1]`), then choose `max(p, np)` and return it, initial call is `solve(n-1)` (_obvious_)
